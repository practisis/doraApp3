...
function agregaretiva(quien){
	...
	//else
	if(retencion != '0' && baseaux!= '0' && baseaux!= '' && basereal > 0 && base > 0){
      ...
      document.getElementById('baseretivareal').value=basequeda.toFixed(4);
		...
		/*
		var newtotal=document.getElementById('totaldebe').value-valor;
	    document.getElementById('totaldebe').value=(Math.round(newtotal * 100)/100).toFixed(2);
		*/
    }
    sacatotal();
...
function RemoveRowIva(tb){
  ...
  /*
  var valor=parseFloat(document.getElementById('valorret'+tb).value);
  var newtotal=parseFloat(document.getElementById('totaldebe').value)+valor;
  document.getElementById('totaldebe').value=(Math.round(newtotal * 100)/100).toFixed(2);
  */
...
function agregaret(cual){
	...
	if(pasaret==true){
		if(retencion != '0' && (baseauxci!= '0' || baseauxsi!= '0') && (baseauxci!= '' || baseauxsi!= '')){
			...
			if(status){
				var total=document.getElementById('totaldebe').value;
				total=total-(Math.round(valor * 100)/100).toFixed(2);
				document.getElementById('totaldebe').value=total;
			}
			if(cual==1){
...
function validarretencion(){
  ...
  var cuentadetracciones=0;
	...
	/*código para guardar las detracciones en la misma tabla de retenciones para perú*/
	if($("#tipotasa").length>0){
		if($('#tipotasa').attr('data-id')>0){
			console.log('tiene detracciones');
			var idret=$('#tipotasa').attr('data-id');
			var nombreret=$('#tipotasa').val();
			var porcentajeret=$('#impdetraccion').val();
			var baseciva=$('#basedetraccion').val();
			var basesiva=0;
			var aux=2; //es retencion de renta por default
			datosdetalleretenciones=idret+'||'+nombreret+'||'+porcentajeret+'||'+baseciva+'||'+basesiva+'||'+aux+'@@';
			cuentadetracciones++;
		}
	}
	
	/*fin código de detracciones*/	
    ...
    }//para las detracciones agregaret
	else if(cuentadetracciones>0){
		var numeroretencion = $('#nrodetraccion').val();
		var ingreso = $('#fechadetraccion').val();
		var sri = '1234567890';
		var serie = '000-000';
		var caduca = ingreso;
		var totalretencion = parseFloat($('#tasadetraccion').val());
		datosretenciones = numeroretencion+'||'+serie+'||'+sri+'||'+ingreso+'||'+caduca+'||'+totalretencion;
		if(numeroretencion==''){
			$('#error-content').html('Debe ingresar el número de la detracción para poder guardar.');
			$('.alert-danger').not('#uploaderrors').slideDown();
			$('#btnguarda').prop('disabled',false);
			$("#content").scrollTop(0);
		}else{
			$('#datosretencion').val(datosretenciones);
			$('#datosdetalleretencion').val(datosdetalleretenciones);
			$('#tieneretencion').val(1);
			console.log('Guarda Detracciones');
			QuitarVectores();
		}
	}
	else{
...
function CalcularSublinea(elemento){
	...
	console.log(suma);
	$('#total_'+partes[1]).val(suma.toFixed(ndec));
	
	$('#firstSubConIva').val(misubciva.toFixed(ndecc));
	$('#firstSubSinIva').val(misubsiva.toFixed(ndecc));
    $('#firstSubCeroIva').val(misubceroiva.toFixed(ndecc));
    /*$('#finalSubConIva').val(misubciva.toFixed(4));
	$('#finalSubSinIva').val(misubsiva.toFixed(4));
    $('#finalSubCeroIva').val(misubceroiva.toFixed(4));*/
	$('#finalIVA').val(miiva.toFixed(ndecc));
    //alert(miiva)
...
function AddLineGasto(){
	...
	clone.find('.idcuentas').attr('data-ccperu','0');
	clone.find('#cuenta_'+lineas+",#idcuenta_"+lineas).val('');
	clone.find('.toolccperu').css('display','none');
	...
	$('.cuentas').each(function(){
		$(this).autocomplete({
			...
			select:function(event,ui){
				...
				if(ui.item.ccperu==true){
					if($('#jsonccperu').val()!=''){
						//alert(ui.item.ccdefault);
						if(ui.item.ccdefault!=null&&parseInt(ui.item.ccdefault)>0){
							$(obj).find('[name="cuentagasto[]"]').attr('data-ccperu',parseInt(ui.item.ccdefault));
						}else{
							var jsonccperu=JSON.parse($('#jsonccperu').val());
							$(obj).find('[name="cuentagasto[]"]').attr('data-ccperu',jsonccperu[0].id);
						}
						$(obj).find('[name="labccperu[]"]').css('display','');
					}
				}else{
					$(obj).find('[name="cuentagasto[]"]').attr('data-ccperu','0');
					$(obj).find('[name="labccperu[]"]').css('display','none');
				}
...
function MisTotales(){
  ...
  var putTotal = ((subtotalConIVA - descuento2) + (subtotalSinIVA - descuento) + parseFloat((subtotalCeroIVA - descuento3)) + IVA + ice + verde)+otros;
  ...
  // $('#finalTotal1').html(arreglar(putTotal)).toFixed(2);
  $('#finalTotal').html(parseFloat(((subtotalConIVA - descuento2) + (subtotalSinIVA - descuento) + parseFloat((subtotalCeroIVA - descuento3)) + IVA + ice + verde)+otros).toFixed(ndec));
  
	if($('#basedetraccion').length>0){
		//alert(subtotalConIVA+subtotalSinIVA+subtotalCeroIVA);
		if($('#asientodetraccion').length==0){
			$('#basedetraccion').val(subtotalConIVA+subtotalSinIVA+subtotalCeroIVA);
			$('#basedetraccion').attr('data-real',subtotalConIVA+subtotalSinIVA+subtotalCeroIVA);
			CalculaDetraccion();
		}
