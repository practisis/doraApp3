...
function agregaretiva(quien){
	//console.log("hi");
    $('[name="debe[]"]').attr('readonly', true);
    $('[name="haber[]"]').attr('readonly', true);
	...
	if(retencion != '0' && baseaux!= '0' && baseaux!= '' && basereal > 0 && base > 0){
      ...
      document.getElementById('baseretivareal').value=basequeda.toFixed(4);
        ...
        var forFix = ((newtotal * 100)/100).toFixed(2);

        document.getElementById('totaldebe').value=(parseFloat(forFix).toFixed(2));
...
function guardaretingreso(){
  ...
  var datosretenciones = '';
  var datosdetalleretenciones = '';
  var datosretencionesdet = '';
  var datosdetalleretencionesdet = '';
  ...
  var cuentadetracciones=0;
	...
	if($("#tipotasa").length>0){
		if($('#tipotasa').attr('data-id')>0){
			console.log('tiene detracciones ventas');
	...
	/*fin código de detracciones*/	
	 //alert(cuenta+'/'+cuentadetracciones);
    if(cuenta > 0){
      var numeroretencion = document.getElementById('numretencion').value;
      var ingreso = document.getElementById('ingresoret').value;
      var sri = document.getElementById('sriret').value;
	  ...
	  var idretencionliga = document.getElementById('idretencionliga').value;

      datosretenciones = numeroretencion+'||'+serie+'||'+sri+'||'+ingreso+'||'+caduca+'||'+totalretencion+'||'+asiento+'||'+idretencionliga;
     // alert(datosretenciones);
      ...
      if(idcliente==''){
		...
		if(numeroretencion==''){
			...
			$('#datosretenciones').val(datosretenciones);
			$('#datosdetalleretenciones').val(datosdetalleretenciones);
