...
function CalcularSublinea(elemento){
    ...
    //alert(ivaaux);
	var suma=parseFloat($('#subsiva_'+partes[1]).val())+(parseFloat($('#subciva_'+partes[1]).val())*(1+ivaaux))+parseFloat($('#subceroiva_'+partes[1]).val());
    ...
    if(cuenta > 0){
     ...
     // alert(datosretenciones);
      ...
      if(idcliente==''){
		...
		var cont=0;
		var elem=$('#tablatarjetas .misabonos');
		var tam=elem.length;
		console.log(tam);
		if(tam>0){
			$(elem).each(function(){
				if($(this).val()=='0.00' || parseInt($(this).val())==0){
