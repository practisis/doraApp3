...
function VerificarProd(){
  var paisaux = $('#idpaisaux').val();
  if(paisaux == '23'){
    if($('#id_sat_prod').val() == '0' || $('#id_sat_prod').val()== ''){
		$('.alert-danger').html("Por favor ingrese el código SAT del producto / serviciol.");
		$('.alert-danger').slideDown();
        return false;
	}else if($('#id_sat_unidad').val() == '0' || $('#id_sat_unidad').val()== ''){
		$('.alert-danger').html("Por favor ingrese el código SAT de la unidad para el producto.");
		$('.alert-danger').slideDown();
        return false;
	}
  }
