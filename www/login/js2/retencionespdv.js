...
function agregaretiva(){
    ...
    if(numeroretencion==''){
      ...
      document.getElementById('baseretivareal').value=basequeda.toFixed(4);
        ...
        myNewCell=myNewRow.insertCell(-1);
        myNewCell.innerHTML='<td><input type="text" name="valorret'+contm+'" id="valorret'+contm+'" class="form-control" style="width: 100%;text-align: right;height:25px;" value="'+valor.toFixed(4)+'" readonly="readonly"/></td>';
        myNewCell=myNewRow.insertCell(-1);
...
function RemoveRowIva(tb){
  ...
  var fila = document.getElementById(tb);
...
function agregaret(cual){
	...
	if(pasaret==true){
		if(retencion != '0' && (baseauxci!= '0' || baseauxsi!= '0') && (baseauxci!= '' || baseauxsi!= '')){
			...
			myNewCell=myNewRow.insertCell(-1);
			myNewCell.innerHTML='<td><input type="text" name="valorret'+contm+'" id="valorret'+contm+'" class="form-control input-sm" style="width: 100%;text-align: right;height:25px;" value="'+valor.toFixed(4)+'" readonly="readonly"/></td>';
			myNewCell=myNewRow.insertCell(-1);
...
function sacatotal(){
  ...
  var total = 0;
    ...
    document.getElementById('totalretencion').value=total.toFixed(4);
    document.getElementById('paymentCheques').innerHTML=total.toFixed(2);
    updateForm(total.toFixed(2));
...
function CalcularSublinea(elemento){
    ...
    //alert(ivaaux);
	var suma=parseFloat($('#subsiva_'+partes[1]).val())+(parseFloat($('#subciva_'+partes[1]).val())*(1+ivaaux));
	...
	$('#firstSubConIva,#finalSubConIva').val(misubciva.toFixed(4));
	$('#firstSubSinIva,#finalSubSinIva').val(misubsiva.toFixed(4));
...
function LigarAutocomplete(){
	$('.cuentas').each(function(){
		$(this).autocomplete({
			source:"subpages/contabilidad/ajaxplan.php",
...
function MisTotales(){
	...
	console.log(ice);
	console.log(verde);
	$('#finalTotal').html(parseFloat((subtotalConIVA - descuento2) + (subtotalSinIVA - descuento) + IVA + ice + verde).toFixed(2));
}
...
function guardaretingreso(){
  ...
  var cuenta = 0;
    ...
    if(cuenta > 0){
      ...
      if(idcliente==''){
        $('#error-content').html('Debe ingresar el cliente en cartera de la retención para poder guardar.');
		$('.alert-danger').not('#uploaderrors').slideDown();
