			...
			for(var k=0;k<mijson.length;k++){
				...
				var displayline='';
				  ...
				  if(mijson[k].id==0){
					...
					var line="<tr class='line_det factura_con_tarjeta_"+mijson[k].id+"' style='display:"+displayline+"'><td style='text-align:left;'><input name='factID[]' value='"+mijson[k].num+"' style='display:none;'/><input name='CuentaCliente[]' value='"+mijson[k].cuentacli+"' style='display:none;'/><input name='factIDReal[]' value='"+mijson[k].id+"' style='display:none;'/>"+mijson[k].num+' '+tarjeta+"</td><td style='text-align:center;'>"+mijson[k].fecha+"</td><td style='text-align:center;'>"+mijson[k].lote+"</td><td style='text-align:right;' id='tot_"+mijson[k].id+"'>"+parseFloat(mijson[k].total).toFixed(2)+"</td><td style='text-align:right;' id='abo_"+mijson[k].id+"'>"+parseFloat(mijson[k].abono).toFixed(2)+"</td><td style='text-align:right;' id='sal_"+mijson[k].id+"'>"+parseFloat(mijson[k].saldo).toFixed(2)+"</td><td style='text-align:center;'><input onclick='this.select();' class='input-sm form-control misabonos' name='abonosval[]' onkeyup='CalcularSaldo("+mijson[k].id+");' value='0.00' id='newabo_"+mijson[k].id+""+kk+"' "+readtarj+"/><div onclick='SaldoTotal("+mijson[k].id+");' class='btn btn-xs btn-primary' style='display: "+vertarj+";'><span class='glyphicon glyphicon-check'></span></div></td><td style='text-align:center;'><input class='input-sm form-control' id='saldof_"+mijson[k].id+""+kk+"' value='"+parseFloat(mijson[k].saldo).toFixed(2)+"' readonly style='text-align:right;' /></td></tr>";
...
function CalcularSaldo(cual){
	...
	$('#saldof_'+cual).val(newsaldo.toFixed(2));
	$('.saldof_'+cual).val(newsaldo.toFixed(2));
	var todosabonos=0;
...
function SaldoTotal(cual){
	$('#newabo_'+cual).val($('#sal_'+cual).html());
	$('.newabo_'+cual).val($('#sal_'+cual).html());
	CalcularSaldo(cual);
}
...
function AgregaraIngreso(){
	...
	$('#totaldebe').val((ingreso+agregar-agregar2).toFixed(2));
	...
	var forFix = (ingreso+agregar-agregar2).toFixed(2);
	$('#totaldebe1').val(arreglar(forFix));
