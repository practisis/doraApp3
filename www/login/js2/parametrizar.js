...
function GuardarFacturacion(){
	...
	$.ajax({
		  method:"POST",
		  url: "ajax/administrador/apiparametrizar.php",
		  data: { rvpass:21,cajaventas:cadena,bd:bd,tarjetas:cadenacards,otras:cadenaotras,cuentaiva:$('#cuentaiva').val(),cuentaser:$('#cuentaser').val(),cuentaprop:$('#cuentaprop').val(),contabonos:cadenaabono,valoriva:miiva,ventasextranjero:vextranjero,cuentaivacomp:$('#cuentaivacomp').val(),cuentaiceven:$('#cuentaiceven').val()}
	}).done(function(response) {
	    //console.log(response);
		if(response=='OK')
